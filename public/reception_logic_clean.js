const API = '';
  ...
  function renderAppointmentTable(list, isOverdue) {
      ...
      if (isOverdue) {
        actionButtons = `
          <div class="flex gap-2 w-full md:w-auto justify-end">
            <button class="bg-teal-500 text-white px-4 py-2 rounded-lg font-medium shadow-sm hover:bg-teal-600 transition-colors" onclick="resolvePastAppointment('${a.id}', 'YES')">Keldi</button>
            <button class="bg-red-100 text-red-600 px-4 py-2 rounded-lg font-medium hover:bg-red-200 transition-colors" onclick="resolvePastAppointment('${a.id}', 'NO')">Kelmadi</button>
          </div>
        `;
      } else {
        actionButtons = `
          <div class="flex gap-2 w-full md:w-auto justify-end">
            <button class="bg-primary text-white px-4 py-2 rounded-lg font-medium shadow-sm hover:bg-primary-container hover:text-on-primary-container transition-colors" onclick="startAppointment('${a.id}', this)">Boshlash</button>
            <button class="bg-surface-container border border-error text-error px-4 py-2 rounded-lg font-medium hover:bg-error-container transition-colors" onclick="deleteAppt('${a.id}')">Bekor</button>
          </div>
        `;
      }
      
      return `
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between p-4 bg-surface-container-lowest border ${isOverdue ? 'border-orange-300 bg-orange-50/50' : 'border-outline-variant/50 hover:border-primary/30'} rounded-xl shadow-sm mb-3 gap-4 transition-all">
          <div class="flex items-center gap-4">
            <div class="w-10 h-10 rounded-full ${isOverdue ? 'bg-orange-100 text-orange-600' : 'bg-primary-fixed text-primary'} flex items-center justify-center font-bold text-lg shrink-0">
              ${pName.charAt(0).toUpperCase()}
            </div>
            <div>
              <p class="font-headline-sm text-[16px] text-on-surface mb-0.5">${pName}</p>
              <a href="tel:${pPhone}" class="text-teal-600 font-medium text-sm flex items-center gap-1 hover:underline">
                <span class="material-symbols-outlined text-[16px]">call</span> ${pPhone}
              </a>
            </div>
          </div>
          ...
          <div class="flex flex-col md:flex-row md:items-center gap-4 md:gap-8 w-full md:w-auto">
            <div>
              <p class="font-label-md text-on-surface">Dr. ${a.doctor.lastName}</p>
              <p class="text-sm text-on-surface-variant">${a.procedure?.name || 'Ko\'rik'}</p>
            </div>
            
            <div class="text-left md:text-right min-w-[100px]">
              <p class="font-label-md ${isOverdue ? 'text-orange-600' : 'text-primary'}">${date}</p>
              <p class="text-sm font-bold ${isOverdue ? 'text-orange-600' : 'text-on-surface-variant'}">${time}</p>
            </div>
    ...
    return `<div class="flex flex-col">${cards}</div>`;
  }
  ...
  function renderMissedTable(list) {
    const cards = list.map(a => {
      ...
      return `
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between p-4 bg-red-50 border border-red-200 rounded-xl shadow-sm mb-3 gap-4">
          <div class="flex items-center gap-4">
            <div class="w-10 h-10 rounded-full bg-red-100 flex items-center justify-center text-red-600 font-bold text-lg shrink-0">
              ${pName.charAt(0).toUpperCase()}
            </div>
            <div>
              <p class="font-headline-sm text-[16px] text-red-600 mb-0.5">${pName}</p>
              <a href="tel:${pPhone}" class="text-red-600 font-bold text-sm flex items-center gap-1 hover:underline">
                <span class="material-symbols-outlined text-[16px]">call</span> ${pPhone}
              </a>
            </div>
          </div>
          ...
          <div class="flex flex-col md:flex-row md:items-center gap-4 md:gap-8 w-full md:w-auto">
            ...
            <div class="text-left md:text-right min-w-[100px]">
              <p class="font-label-md text-red-600">${date}</p>
              <p class="text-sm font-bold text-red-600">${time}</p>
            </div>

            <div class="flex flex-wrap gap-2 justify-end w-full md:w-auto">
              <button class="bg-emerald-500 text-white px-4 py-2 rounded-lg font-medium shadow-sm hover:bg-emerald-600 transition-colors flex items-center gap-1" onclick="openContactedModal('${a.id}', '${encodeURIComponent(JSON.stringify(a))}')">
                <span class="material-symbols-outlined text-[18px]">call</span> Bog'lanildi
              </button>
              <button class="bg-amber-500 text-white px-4 py-2 rounded-lg font-medium shadow-sm hover:bg-amber-600 transition-colors" onclick="updateCallStatus('${a.id}', 'NO_ANSWER')">
  ...
  function renderResolvedMissedTable(list) {
    const cards = list.map(a => {
      ...
      return `
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between p-4 bg-emerald-50 border-l-4 border-l-emerald-500 border-y border-r border-emerald-100 rounded-xl shadow-sm mb-3 gap-4">
          <div class="flex items-center gap-4">
            <div class="w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-600 font-bold text-lg shrink-0">
              ${pName.charAt(0).toUpperCase()}
            </div>
            <div>
              <p class="font-headline-sm text-[16px] text-emerald-700 line-through mb-0.5">${pName}</p>
              <span class="text-emerald-600 font-medium text-sm flex items-center gap-1">
                <span class="material-symbols-outlined text-[16px]">call</span> ${pPhone}
              </span>
            </div>
          </div>
          ...
          <div class="flex flex-col md:flex-row md:items-center gap-4 md:gap-8 w-full md:w-auto">
            <div>
              <p class="font-label-md text-emerald-800">Dr. ${a.doctor.lastName}</p>
              <p class="text-sm text-emerald-600/80">${a.procedure?.name || 'Ko\'rik'}</p>
            </div>
            
            <div class="text-left md:text-right min-w-[100px]">
              <p class="font-label-md text-emerald-700">${date}</p>
              <p class="text-sm font-bold text-emerald-700">${time}</p>
            </div>

            <div class="flex justify-end w-full md:w-auto">
              <span class="bg-emerald-500 text-white px-4 py-2 rounded-lg font-bold text-sm shadow-sm flex items-center gap-1">
                <span class="material-symbols-outlined text-[18px]">check_circle</span> Bog'lanildi
              </span>
            </div>
          </div>
        </div>
  ...
  function renderUnreachableMissedTable(list) {
    const cards = list.map(a => {
      ...
      return `
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between p-4 bg-slate-50 border-l-4 border-l-slate-400 border-y border-r border-slate-200 rounded-xl shadow-sm mb-3 gap-4">
          <div class="flex items-center gap-4">
            <div class="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center text-slate-600 font-bold text-lg shrink-0">
              ${pName.charAt(0).toUpperCase()}
            </div>
            <div>
              <p class="font-headline-sm text-[16px] text-slate-500 line-through mb-0.5">${pName}</p>
          ...
          <div class="flex flex-col md:flex-row md:items-center gap-4 md:gap-8 w-full md:w-auto">
            <div>
              <p class="font-label-md text-slate-600">Dr. ${a.doctor.lastName}</p>
              <p class="text-sm text-slate-400">${a.procedure?.name || 'Ko\'rik'}</p>
            </div>
            
            <div class="text-left md:text-right min-w-[100px]">
              <p class="font-label-md text-slate-500">${date}</p>
              <p class="text-sm font-bold text-slate-500">${time}</p>
            </div>

            <div class="flex justify-end w-full md:w-auto">
              <span class="bg-slate-500 text-white px-4 py-2 rounded-lg font-bold text-sm shadow-sm flex items-center gap-1">
                <span class="material-symbols-outlined text-[18px]">volume_off</span> Bog'lanib bo'lmadi
              </span>
            </div>
          </div>
        </div>
  ...
  function renderCancelledTable(list) {
    const cards = list.map(a => {
      ...
      return `
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between p-4 bg-orange-50 border-l-4 border-l-orange-500 border-y border-r border-orange-200 rounded-xl shadow-sm mb-3 gap-4">
          <div class="flex items-center gap-4">
            <div class="w-10 h-10 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 font-bold text-lg shrink-0">
              ${pName.charAt(0).toUpperCase()}
            </div>
            <div>
              <p class="font-headline-sm text-[16px] text-orange-700 line-through mb-0.5">${pName}</p>
              <span class="text-orange-600 font-medium text-sm flex items-center gap-1">
                <span class="material-symbols-outlined text-[16px]">call</span> ${pPhone}
              </span>
            </div>
          </div>
          ...
          <div class="flex flex-col md:flex-row md:items-center gap-4 md:gap-8 w-full md:w-auto">
            <div>
              <p class="font-label-md text-orange-800">Dr. ${a.doctor.lastName}</p>
              <p class="text-sm text-orange-600/80">${a.procedure?.name || 'Ko\'rik'}</p>
            </div>
            
            <div class="text-left md:text-right min-w-[100px]">
              <p class="font-label-md text-orange-700">${date}</p>
              <p class="text-sm font-bold text-orange-700">${time}</p>
            </div>

            <div class="flex flex-col text-sm text-orange-900 bg-white/50 p-2 rounded-lg border border-orange-200 w-full md:w-64">
              <div><strong>Bekor qildi:</strong> ${canceller}</div>
              <div class="italic truncate" title="${note}"><strong>Sabab:</strong> ${note}</div>
            </div>
          </div>
        </div>
      `;
  ...
  function renderActiveVisits() {
    ...
    if (activeVisits.length === 0) {
      ...
      const emptyStateHtml = `
        <div class="col-span-full flex flex-col items-center justify-center p-12 bg-surface border border-dashed border-outline-variant rounded-xl text-on-surface-variant">
            <span class="material-symbols-outlined text-[48px] mb-4 opacity-50">chair</span>
            <p class="font-headline-sm">Hozirda muolajada bemorlar yo'q</p>
        </div>
      `;
      if(container) container.innerHTML = emptyStateHtml;
      
      if(inProgressContainer) inProgressContainer.innerHTML = '<div class="py-6 text-center text-on-surface-variant text-sm flex flex-col items-center gap-2"><span class="material-symbols-outlined text-[24px] opacity-50">chair</span> Hozirda muolajada bemorlar yo\'q</div>';
      return;
    }
    ...
    const htmlCards = activeVisits.map(v => `
      <div class="bg-secondary-container/30 border border-secondary/30 rounded-xl p-4 shadow-sm relative overflow-hidden transition-all hover:shadow-md" id="card-${v.id}">
        <div class="absolute top-0 right-0 w-16 h-16 bg-secondary/10 rounded-bl-full -z-10"></div>
        <div class="absolute top-4 right-4 w-2.5 h-2.5 rounded-full bg-secondary animate-pulse"></div>
        <div class="flex flex-col gap-3">
          <div>
            <div class="font-headline-sm text-on-surface text-[16px] pr-8">${v.patientName}</div>
            <div class="text-sm font-medium text-secondary-dark mt-0.5">Dr. ${v.doctor.lastName}</div>
            <div class="text-xs text-on-surface-variant">${v.serviceName}</div>
          </div>
          <div class="flex justify-between items-center mt-2 border-t border-secondary/20 pt-3">
            <div class="timer font-mono text-sm font-bold text-secondary-dark flex items-center gap-1.5" id="timer-${v.id}" data-end="${v.endTime || ''}">
              <span class="material-symbols-outlined text-[16px]">hourglass_top</span> --:--
            </div>
            <button class="bg-secondary text-on-secondary px-4 py-1.5 rounded-lg text-sm font-bold shadow-sm hover:bg-secondary-dark transition-colors" onclick="finishVisit('${v.id}')">Tugatish</button>
          </div>
        </div>
      </div>
    `).join('');
  ...
  async function loadAvailableSlots() {
    ...
    try {
      ...
      listEl.innerHTML = sortedDays.map(dayIso => {
        ...
        const formattedDate = `${dObj.getDate()} ${['Yan', 'Fev', 'Mar', 'Apr', 'May', 'Iyun', 'Iyul', 'Avg', 'Sen', 'Okt', 'Noy', 'Dek'][dObj.getMonth()]}`;
        ...
        if (dayIso === tashkentToday) {
          ...
          const weekday = ['Yakshanba', 'Dushanba', 'Seshanba', 'Chorshanba', 'Payshanba', 'Juma', 'Shanba'][dObj.getDay()];
