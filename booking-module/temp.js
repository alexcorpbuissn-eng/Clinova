    ...
    async function verifyAuthOnLoad() {
      ...
      try {
        ...
        // Network error — stay on login, don't clear token
      }
    }
    verifyAuthOnLoad();
    ...
    async function submitLeave() {
      ...
      try {
        ...
        if(res.success) {
          showAdminToast(`Dam olish saqlandi. ${res.deletedSlots} ta bo'sh slotlar o'chirildi!`);
          closeLeaveModal();
          loadLeaves();
        } else {
    ...
    async function deleteLeave(id) {
      if(!confirm("Haqiqatan ham ushbu dam olishni bekor qilmoqchimisiz?")) return;
      try {
        const res = await apiDelete(`/api/admin/leaves?id=${id}`);
        if(res.success) {
          showAdminToast("Bekor qilindi");
          loadLeaves();
        }
      ...
      try {
        ...
        if (id) {
          res = await fetch(`/api/admin/doctors/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('admin_token')}` },
            body: JSON.stringify({ firstName, lastName, specialty, bio, photoUrl })
          }).then(r => r.json());
        } else {
          res = await apiPost('/api/admin/doctors', { firstName, lastName, specialty, bio, photoUrl });
        }

        if(res.success) {
          showAdminToast("Muvaffaqiyatli saqlandi!");
          closeDoctorModal();
          loadDoctors();
        } else {
          alert("Xatolik: " + res.error);
    ...
    async function toggleDoctorStatus(id, isActive) {
      ...
      try {
        ...
        if(res.success) {
          showAdminToast("Holat o'zgartirildi!");
          loadDoctors();
        }
      } catch(e) { alert("Xatolik"); }
    }
